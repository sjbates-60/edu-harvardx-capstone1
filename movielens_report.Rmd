...
-   The `genres` field is a list of `|`-separated genres. There are 19 different
    ...
    variables, each named for a genre. A genre variable contains a 1 if the
    movie has the variable name in its `genres` variable, and a 0 otherwise.
...
linear combination of user and movie variables. Determining the effect of
individual genres is especially important, as it allows a rating to be predicted
for a movie whose combination of genres has not been previously rated by a user.
