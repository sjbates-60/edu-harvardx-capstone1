...
on the training set, listed in order of descending RMSE. Examining the models in
the pairs described in the methods section above, I found that the model where
the user effect was not regularized performed slightly better than the modeI
where the user effect was regularized. (The pairs are indicated by the striping
in Table 4.) The improvement in each case was very small, however. The models
that performed best included movie, user, and age effects. These still showed at
most a `r nongenre_improv`% improvement over the model with only movie and user
effects.
