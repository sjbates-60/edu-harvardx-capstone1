...
models for predicting ratings for specific movies by specific people. To test
the models, I used 90% of the ratings and compared their accuracy of their
predictions. Once I had chosen a final model, I used it to predict the ratings
on the final 10% of the ratings and reported how well it succeeded.
...
movie's average rating and the number of times it was rated. This make intuitive
sense; more people will tend to go to movies that they hear have good reviews.
Figure 2 shows that there is a negative correlation between a user's average
rating and the number of movies rated by that user. One can imagine that such a
user is harder to impress, having seen so many movies.
...
model's predicted values from the actual values, and created genre variables to
model the result.

## Models without Genre

The non-genre variables are `userId`, `movieId`, `year`, and `timestamp`.[^1]
Additionally, I calculated an age variable. The age is a measure of how old the
movie was when the user rated it. I calculated it as
`(year(timestamp) + month(timestamp) / 13) - year`. I divided by 13 instead of
by 12 so that the integral part of the age would equal the year of the rating
(otherwise, ratings in December would have the integral part equal to the next
year). I chose the month for the fraction rather than the day or week of the
year, as it exhibited a smoother rate of change.

[^1]: I left out `title` as I could not think of a useful distance function
    between titles.

Furthermore, I added regularization to the models to reduce the effect of movies
with few ratings. Using these variables, I tested eight models, grouped in pairs
that differed only in whether the user effects were regularized:

-   Movie and user effects, with either both regularized or only movie effects
    regularized;

-   Movie, user, and year effects, with either all three regularized or only
    movie and year effects regularized;

-   Movie, user, and age effects, with either movie and user effects regularized
    or only movie effects regularized;

-   Movie, user, and age effects, with either all three regularized or only
    movie and age effects regularized.
...
estimate from the first phase. Again, I chose a linear combination of genre
variables as the model. To get the genre variables, I separated the `genres`
variable into 20 separate variables, one for each of the genres in Table 3. For
display purposes, I abbreviated each genre to two characters. Each genre
variable has a value of 1 if the genre appears in the movie's `genres` variable
...
above. The exploration then becomes a calculation of the term $e_g^u$, the
effect of genre $g$ on user $u$, for each user and genre.
...
To compute $\hat{Y}_{m,u}$ efficiently, note that the sum above can be rewritten
as the dot product of the vectors $(x_m^1, \ldots , x_m^{20})$ and
...
Table 4 shows the model name, the optimal tuning value $\lambda$, and the RMSE
...
The improvement in each case was very small, however. The models that performed
best included movie, user, and age effects. These still showed at most a
`r improv(6, 1, 1)`% improvement over the model with only movie and user
effects.

```{r nongenre-model-results}
movie_results <- readRDS("model_results.rds")
...
Adding the genre effects with $\lambda_2 = 1.8$ to the best non-genre model did
produce a significant improvement, however, of `r improv(9, 6, 1)`%.

```{r genre-model-results}
knitr::kable(model_results[9, ],
             digits = 6,
             col.names = c("Model name", "$\\lambda$", "$\\lambda_2$", "RMSE"),
             caption = "Result of combined models on the training set")
```

Finally, combining the two models and running them on the validation set yielded
an RMSE value of `r round(model_results[10, "RMSE"], 6)`.

# Conclusion

Modelling only good for static analysis. To make it a recommendation system, I
would need to keep more data on each user. Minimally, this would be the number
of ratings for each user and genre, so that the average ratings could be
appropiately updated when a user rates a new movie.

Limits due to data? Maybe show how variable a user can be within genre
combinations?
